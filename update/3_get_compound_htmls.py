...
def combine_compounds(dir_root: str) -> None:
    '''Combines csv-files containing compounds.

    If a user-local WebBook index is available, it is used as one source of
    already known compound IDs. NistChemPy no longer ships a package-internal
    compound table.
    '''

    try:
        df0 = nist.get_local_index().to_dataframe()
    except nist.NistChemPyIndexNotFoundError:
        df0 = pd.DataFrame({'ID': [], 'inchi': []})
